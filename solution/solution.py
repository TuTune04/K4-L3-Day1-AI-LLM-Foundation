...
def call_openai(
    ...
    # Hỗ trợ cả GEMINI_API_KEY hoặc OPENAI_API_KEY
...
def compare_models(prompt: str) -> dict:
    ...
    # Lấy giá output theo model hiện tại (fallback về 'gpt-4o' nếu chạy test cũ)
    pricing = PRICING_PER_1K_TOKENS.get(
        OPENAI_MODEL, PRICING_PER_1K_TOKENS.get("gpt-4o", {"output": 0.010})
    )
...
def chat_with_system_prompt(
    ...
    from openai import OpenAI  # import BÊN TRONG hàm
    # Ưu tiên GEMINI_API_KEY nếu có, fallback OPENAI_API_KEY
    api_key = os.getenv("GEMINI_API_KEY") or os.getenv("OPENAI_API_KEY")
    
    # Nếu dùng GEMINI_API_KEY thì truyền base_url, nếu test mock bằng OPENAI thì không ép base_url
...
def streaming_chatbot() -> None:
    ...
    from openai import OpenAI  # import BÊN TRONG hàm theo quy định lab
    ...
    while True:
        ...
        # Gọi API stream với toàn bộ history tích lũy
        ...
        # Lưu phản hồi của assistant và chỉ giữ 3 lượt gần nhất (tối đa 6 tin nhắn)
...
def run_assistant(
    ...
    while True:
        ...
        stream = retry_with_backoff(call_stream)
        ...
        # Cập nhật lịch sử và giữ tối đa 3 lượt (6 messages)
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": assistant_reply})
        history = history[-6:]

        # Cộng dồn thống kê lượt chat
        num_turns += 1
        turn_cost_data = estimate_cost(user_msg, assistant_reply, model=OPENAI_MODEL)
        total_tokens += turn_cost_data["input_tokens"] + turn_cost_data["output_tokens"]
        total_cost += turn_cost_data["total_cost"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        table += f"| {result['prompt']} | {result['gpt4o_response'][:40]}... | {result['mini_response'][:40]}... | {result['gpt4o_latency']:.2f}s | {result['mini_latency']:.2f}s |\n"
    return table
